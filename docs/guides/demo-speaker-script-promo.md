...
**Opción B: paso a paso con control humano (ejemplo típico de `.claude/commands/run.md`)**
...
| ---------------- | ------------------------------- | --------------------------------- |
| Cómo arranca     | `npx ts-node src/cli/run.ts ...`| Chat o `/run`                     |
...
# Modo .claude
# En Claude Code escribir: /run --job /tmp/demo-promo-job.json --approve
