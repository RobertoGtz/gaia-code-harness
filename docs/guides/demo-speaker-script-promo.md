...
**Opción B: paso a paso con control humano (ejemplo típico de `.claude/commands/run_gaia.md`)**
...
| ---------------- | ------------------------------- | --------------------------------- |
| Cómo arranca     | `npx ts-node src/cli/run.ts ...`| Chat o `/run_gaia`                     |
...
# Modo .claude
# En Claude Code escribir: /run_gaia --job /tmp/demo-promo-job.json --approve
