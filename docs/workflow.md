# El flujo Uncle Bob — GAIA Code Harness

> El harness soporta dos modos de orquestación simultáneamente.
> Este documento describe el modo **Claude Code** (artesano) y cómo se
> mapea al modo **HTTP + Postgres** (producción).
>
...
| Gherkin       | `gherkin_author`    | *(parte de SpecAuthorAgent)*  | Separado en Claude mode |
| Implementación| `tdd_craftsman`     | `ImplementerAgent.executeTDD()` | Activo si `tddMode: true` |
| *(bulk)*      | *(bulk implementer)*| `ImplementerAgent.execute()`  | `tddMode: false` |
