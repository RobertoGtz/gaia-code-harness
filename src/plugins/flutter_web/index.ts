...
- Routing: fluro only. Routes are defined in ${base}/lib/src/core/${feature}_routes.dart. Concrete FLOW controllers (in lib/src/presentation/flow/<module>/) use appManager.router.navigateTo(route, transition: TransitionType.fadeIn, clearStack: true/false) and appManager.router.navigateBack(), importing 'package:fluro/fluro.dart' for TransitionType. The abstract MODULE controller (lib/src/presentation/modules/<module>/<module>_module_controller.dart) must ONLY declare abstract navigation methods; it must NOT import fluro, implement navigation, or use TransitionType.
...
- Tests: ${base}/test/src/ using mocktail + flutter_test
    ...
    * Controller tests: import the concrete controller from "package:${feature}/${feature}_core.dart". Also import 'package:hooks_riverpod/hooks_riverpod.dart' and 'package:pay_multiplatform_common/pay_multiplatform_common.dart' in every controller test because MockRef implements Ref and MockAppManager implements AppManager. Check whether test/src/mocks/mocks.dart (or test/src/mocks.dart) exists; if it does NOT exist, define local MockRef, MockAppManager, MockAppRouter and MockAppLogger classes in the test file using mocktail. NEVER import a relative mocks path that does not exist.
    * The module widget pattern-matches states and calls EXISTING controller action methods (e.g. loadPresummary, navigateToSummary, onBackPressed). Do NOT add controller methods named handleSuccess, handleError, onSuccess, onError, or similar state-branch handlers.
    * For retry actions on error states, the module widget calls the EXISTING controller.loadPresummary(). Do NOT add a separate retry method to the controller abstract or concrete class.
    * Controller tests verify EXISTING action methods only. Do NOT invent new controller methods in tests. If the existing test already covers the required action (navigation target, notifier call), do not add duplicate tests.
    * Controller tests must NOT stub, read, or assert on notifier state or view state values. Do NOT use mockNotifier.state, presummaryFormViewStateProvider state, or SummaryFormViewStates constructors in controller tests. The controller is stateless; only verify notifier method calls and router navigation.
    * Local MockAppManager pattern (AVOID the recursive getter bug): class MockAppManager extends Mock implements AppManager \{ final MockAppRouter router = MockAppRouter(); \}. Do NOT write AppRouter get router => router; or AppRouter get router => _router;.
    ...
    * If the controller test file already exists, READ IT FIRST and only ADD new test cases. PRESERVE all existing imports, mock classes, setUp, and helper methods. Do NOT rewrite the file from scratch and do NOT add a relative mocks.dart import unless it already exists.
...
- KNOWN CASHFLOW REPOS (rpp-co/rpp-cashflow-multiplatform-pyme):
    ...
    * bre_b notifier class: PresummaryFormStatesNotifier extends StateNotifier<SummaryFormViewStates> with SafeStateNotifier (method loadPresummary lives here); import from package:bre_b/src/presentation/modules/presummary_form/presummary_form_states_notifier.dart when mocking it in tests
